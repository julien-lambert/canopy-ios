...
struct GardenMapScreen: View {
    ...
    enum Mode {
        case view
        case place(speciesID: Int? = nil)
        case edit(plantID: Int)
        case pick(
            initialCoordinate: CLLocationCoordinate2D? = nil,
            onPick: (CLLocationCoordinate2D) -> Void
        )
    }

    private enum ActiveSheet: Identifiable {
        case individual(Int)
        case create(PlantFormPrefill)

        var id: String {
            switch self {
            case .individual(let plantID):
                return "individual-\(plantID)"
            case .create(let prefill):
                let latitude = prefill.latitude.map { String(format: "%.6f", $0) } ?? "nil"
                let longitude = prefill.longitude.map { String(format: "%.6f", $0) } ?? "nil"
                return "create-\(latitude)-\(longitude)-\(prefill.speciesID ?? -1)"
            }
        }
    }
    ...
    var body: some View {
        ...
        .sheet(item: $activeSheet) { sheet in
            ...
            case .individual(let plantID):
                IndividualSheet(
                    mode: .detail(plantID: plantID),
                    onRequestMove: { requestedPlantID in
                        if let pin = pins.first(where: { $0.id == requestedPlantID }) {
                            beginEditing(pin)
                        }
                    }
                )
    ...
    private var placementControlsVisible: Bool {
        switch mode {
        case .pick, .place:
            return editingDraft == nil
        case .view, .edit:
            return false
        }
    }
    ...
    private var navigationTitle: String {
        switch mode {
        case .view, .edit:
            return "Carte du jardin"
        case .pick:
            return "Choisir une position"
        case .place:
            return "Placer un individu"
        }
    }

    private var placementControls: some View {
        let coordinate = placementCoordinate
        return CanopyCard(
            title: placementCardTitle,
            subtitle: placementCardSubtitle,
            systemImage: placementSystemImage
        ) {
            VStack(alignment: .leading, spacing: 10) {
                ...
                HStack(spacing: 10) {
                    ...
                    Button(placementPrimaryButtonTitle) {
                        confirmPlacement()
                    }
                    .canopyPrimaryActionStyle()
                }
    ...
    private var placementCardTitle: String {
        switch mode {
        case .pick:
            return "Positionner sur la carte"
        case .place:
            return "Planter ici"
        case .view, .edit:
            return ""
    ...
    private var placementPrimaryButtonTitle: String {
        switch mode {
        case .pick:
            return "Utiliser ce point"
        case .place:
            return "Créer l’individu ici"
        case .view, .edit:
            return ""
        }
    }
    ...
    private var placementCoordinate: CLLocationCoordinate2D {
        ...
        return initialCenterCoordinate()
    }

    private func initialCenterCoordinate() -> CLLocationCoordinate2D {
    ...
    private func activateInitialModeIfNeeded() {
        guard editingDraft == nil else { return }

        if case .edit(let plantID) = mode,
           let pin = pins.first(where: { $0.id == plantID }) {
            beginEditing(pin)
        }
    }

    private func handlePinTap(_ pin: PlantPin) {
        switch mode {
        case .pick, .place:
            return
        case .view, .edit:
    ...
    private func handlePinLongPress(_ pin: PlantPin) {
        switch mode {
        case .pick, .place:
            return
        case .view, .edit:
    ...
    private func selectNearestPin(to coordinate: CLLocationCoordinate2D) {
        ...
        if selectedPin?.id == bestHit.pin.id {
            activeSheet = .individual(bestHit.pin.id)
        } else {
...
struct StrataFilterBar: View {
    @Binding var selected: GardenMapScreen.StrataFilter

    var body: some View {
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 8) {
                ForEach(GardenMapScreen.StrataFilter.allCases) { filter in
                    CanopySelectableChip(
                        title: filter.label,
...
private struct GardenMapUIKitView: NSViewRepresentable {
    let pins: [GardenMapScreen.PlantPin]
    let selectedStrataFilter: GardenMapScreen.StrataFilter
    ...
    let mapBaseStyle: GardenMapScreen.MapBaseStyle
    ...
    private func syncPins(on mapView: MKMapView, context: Context) {
        ...
        let effectivePins: [GardenMapScreen.PlantPin] = filtered.map { pin in
            guard let draft = editingDraft, draft.id == pin.id else { return pin }
            return GardenMapScreen.PlantPin(
                id: pin.id,
                commonName: pin.commonName,
    ...
    private func makeBaseTileOverlay(for style: GardenMapScreen.MapBaseStyle) -> MKTileOverlay? {
        switch style {
        case .photo, .planNB:
            return nil
        case .ignPlan:
...
private final class MacPlantPointAnnotation: MKPointAnnotation {
    let pin: GardenMapScreen.PlantPin
    init(pin: GardenMapScreen.PlantPin) {
        self.pin = pin
        super.init()
    }
}
...
private struct GardenMapUIKitView: UIViewRepresentable {
    ...
    private func syncPins(on mapView: MKMapView, context: Context) {
        ...
        let effectivePins: [GardenMapScreen.PlantPin] = filteredPins.map { pin in
            guard let draft = editingDraft, draft.id == pin.id else { return pin }
            return GardenMapScreen.PlantPin(
                id: pin.id,
                commonName: pin.commonName,
    ...
    private func makeBaseTileOverlay(for style: GardenMapScreen.MapBaseStyle) -> BaseTileOverlay? {
        switch style {
        case .photo, .planNB:
            return nil
        case .ignPlan:
    ...
    final class Coordinator: NSObject, MKMapViewDelegate {
        ...
        private var activeDraggedPin: GardenMapScreen.PlantPin?
